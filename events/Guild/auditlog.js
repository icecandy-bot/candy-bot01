...
client.on("messageUpdate", (oldMessage, newMessage) => {
  if (oldMessage.author.bot) return; //developer by tn_hazem
  ...
  client.on("guildBanAdd", async ban => {
    ...
    const fetchedLogs = await ban.guild.fetchAuditLogs({
      limit: 1,
      type:"MEMBER_BAN_ADD",//developer by tn_hazem
    });
  ...
  client.on("channelDelete" , async channel => {
    ...
    const fetchedLogs = await channel.guild.fetchAuditLogs({
      limit: 1,
      type:"CHANNEL_DELETE"//developer by tn_hazem
    });
...
client.on("guildMemberUpdate", async (oldMember, newMember) => {
  ...
  if (chAdd || chRemove) {//developer by tn_hazem
    ...
    if (addedRoles.size > 0) {
      const embedAdd = new MessageEmbed()
        ...
        .setAuthor({//developer by tn_hazem
    ...
    if (removedRoles.size > 0) {
      const embedRemove = new MessageEmbed()
        ...
        .setFooter({//developer by tn_hazem
          text: `${newMember.guild.name}`,
          iconURL: `${newMember.guild.iconURL()}`,
        })
        .setThumbnail(`${newMember.user.displayAvatarURL()}`)
  ...
  client.on("roleUpdate" , async (oldRole , newRole) => {
    ...
    const embed = new Discord.MessageEmbed()
    .setTitle("Role Uptaded !")//developer by tn_hazem
      .setColor('RANDOM')
